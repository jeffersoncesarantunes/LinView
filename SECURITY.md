...
- **Rate limiting is in-memory per-worker**: The `LINSPEC_RATE_LIMIT` is enforced per gunicorn worker process, not shared across workers. Under high load, a client could send up to `workers × RATE_LIMIT` requests per minute.
...
- **SQLite concurrency**: SQLite with WAL mode handles concurrent reads well, but concurrent writes may cause `database is locked` errors under heavy parallel submission. Use PostgreSQL as an alternative for high-throughput deployments.
